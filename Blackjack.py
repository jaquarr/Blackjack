...
if wager >= 2 and wager <= 500: #case for when the wager is legal
    ...
    for i in range(1,53):
        deck.append(i)
        shuffle(deck)
    card1P1 = deck[0] #first card drawn for the user
    card2P1 = deck[1] #second card drawn for the user
    scoreP1 = 0 #allows the score to be tallied
    if deck[0] == 1 or deck[0] == 2 or deck[0] == 3 or deck[0] == 4: #if cases for 
    ...
    deck.remove(deck[0])
    deck.remove(deck[1])
    input('Hit or stand? Type "h" to hit and "s" to stand. ')
    print()
